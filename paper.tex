...
In \autoref{lst:corporate_1} Alice less Emma  specify whether an \code{App} (a variable) \code{isRunnable};
she allows her to delegate the decision further (\code{can-say}\/~\code{inf}).
Emma specifies her concerns as policies to be met in \autoref{lst:corporate_2}:
  if Emma is convinced all these are met then she will say the \code{App isRunnable}.
In \autoref{lst:corporate_3} and \autoref{lst:corporate_4} Emma specifies that an app meets the \code{reputable-policy} if the \code{App isBuyable};
  with \code{"google-play"} as the decider of what is buyable or not.
...
she needs to collect statements to show the app meets the \code{isRunnable} predicate.
Namely:
\begin{itemize}
...
\begin{figure}\centering
  \subfigure[Uptake of Lin~\etal policies.]{%
